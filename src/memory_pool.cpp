...
void MemoryPool::createMemoryBlock(size_t block_size) {
    ...
    SMemoryBlockHeader* block =
        reinterpret_cast<SMemoryBlockHeader*>(std::malloc(sizeof(SMemoryBlockHeader) + block_size));
    if (block == NULL)
...
void* MemoryPool::allocate(size_t size) {
    ...
    SMemoryUnitHeader* unit = reinterpret_cast<SMemoryUnitHeader*>(
        reinterpret_cast<char*>(this->currentBlock) + sizeof(SMemoryBlockHeader) + this->currentBlock->offset);
    unit->length = size;
...
void* MemoryPool::reallocate(void* unit_pointer_start, size_t new_size) {
    ...
    SMemoryUnitHeader* unit =
        reinterpret_cast<SMemoryUnitHeader*>(reinterpret_cast<char*>(unit_pointer_start) - sizeof(SMemoryUnitHeader));
